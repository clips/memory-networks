...
def process_data_clicr_kv(args, log):
    ...
    data, val_data, test_data, vocab = load_data_clicr_kv(args.data_dir, args.ent_setup, log, args.win_size_kv, args.max_n_load, args.which_test, ent_in_ctx, args.remove_notfound)
...
def process_data_clicr_kvpointer(args, log):
    ent_in_ctx = True  # in keys, use entities as contexts. False will use words
    data, val_data, test_data, vocab = load_data_clicr_kv(args.data_dir, args.ent_setup, log, args.win_size_kv, args.max_n_load, args.which_test, ent_in_ctx, args.remove_notfound)
    ...
    '''
    memory_size, k_size, v_size, vocab_size, word_idx, output_size = calculate_parameter_values_clicr_kvpointer(data=data, debug=args.debug,
                                                                                        memory_size=args.memory_size,
                                                                                        vocab=vocab, log=log)
    if args.debug:
        log.info("Vocabulary Size: {}".format(vocab_size))
    ...
    return data, val_data, test_data, k_size, v_size, vocab_size, memory_size, word_idx, output_size
...
def load_data_clicr_kv(data_dir, ent_setup, log, win_size=3, max_n_load=None, which_test=None, ent_in_ctx=True, remove_notfound=False):
    #train_data, _ = load_clicr_ent_only(data_dir + "train1.0.json", ent_setup, max_n_load=max_n_load)
    train_data = load_clicr_kv(data_dir + "train1.0.json", win_size=win_size, ent_setup=ent_setup, max_n_load=max_n_load, ent_in_ctx=ent_in_ctx, remove_notfound=remove_notfound)
    val_data = load_clicr_kv(data_dir + "dev1.0.json", win_size=win_size, ent_setup=ent_setup, remove_notfound=remove_notfound, max_n_load=max_n_load, ent_in_ctx=ent_in_ctx)
    if which_test == "seen":
        test_data = load_clicr_kv(data_dir + "test_seen1.0.json", win_size=win_size, ent_setup=ent_setup,
                                  remove_notfound=remove_notfound, max_n_load=max_n_load, ent_in_ctx=ent_in_ctx)
    elif which_test == "unseen":
        test_data = load_clicr_kv(data_dir + "test_unseen1.0.json", win_size=win_size, ent_setup=ent_setup, remove_notfound=remove_notfound, max_n_load=max_n_load, ent_in_ctx=ent_in_ctx)
    else:
        test_data = load_clicr_kv(data_dir + "test1.0.json", win_size=win_size, ent_setup=ent_setup, remove_notfound=remove_notfound, max_n_load=max_n_load, ent_in_ctx=ent_in_ctx)
    data = train_data + val_data + test_data  # TODO exclude test?
...
def calculate_parameter_values_clicr_kvpointer(data, debug, memory_size, vocab, log):
    word_idx = dict((c, i + 1) for i, c in enumerate(vocab))
    max_story_size = max(map(len, (k for (k,v), _, _, _, _, _ in data)))
    mean_story_size = int(np.mean(list(map(len, (k for (k,v), _, _, _, _, _ in data)))))
    k_size = max(map(len, chain.from_iterable(k for (k,v), _, _, _, _, _ in data)))
    ...
    if debug is True:
        ...
        log.info("Average story length: {}".format(mean_story_size))
        log.info("Memory size: {}".format(memory_size))
    return memory_size, k_size, v_size, vocab_size, word_idx, output_size
...
def vectorize_data_clicr_kvpointer(data, word_idx, output_size, k_size, memory_size):
    '''
    Vectorize stories (into keys and values) and queries.
    ...
    for (k,v), query, answer, _, _, id in data:
        lq = max(0, k_size - len(query))
        q = [word_idx[w] for w in query] + [0] * lq
        ...
        if len(ks) > memory_size:
            ...
            least_like_q = sorted(ks, key=functools.cmp_to_key(
                lambda x, y: jaccard_similarity_score((x), q_words) < jaccard_similarity_score((y), q_words)))
            max_least_like = len(ks) - memory_size
            assert max_least_like > 0
            for sent in least_like_q:
                # Remove the first occurrence of sent. A list comprehension as in [sent for sent in ss if sent not in least_like_q]
                # should not be used, as it would remove multiple occurrences of the same sentence, some of which might actually make the cutoff.
                del_id = ks.index(sent)
                # dirty hack: we need only those questions answered in the passage,
                # don't want jaccard similarity to leave us with no sentence containing the answer
                if vs[del_id] == word_idx[answer[0]]: # don't remove it it removed the answer entity
                    continue
                del ks[del_id]
                del vs[del_id]
                max_least_like -= 1
                if max_least_like == 0:
                    break
            assert len(ks) == len(vs) == memory_size
            p_m = [1.] * memory_size
        else:
            # pad to memory_size
            lm = max(0, memory_size - len(ks))
        ...
        y = np.zeros(output_size)
        for a in answer:
            for c, val in enumerate(vs):
                if val == 0:
                    continue
                if inv_w_idx[val] == a:
                    y[c] = 1
        if np.sum(y) < 1:
            print("no answer in passage for the question (removed due to pruning for memory cache). skipping")
            continue
        # att supervision
        assert len(answer) == 1
        y_att = np.zeros(memory_size)
    ...
    return np.array(K), np.array(V), np.array(Q), np.array(A), np.array(PM), np.array(KM), np.array(QM), np.array(AA)
...
def vectorized_batches_kvpointer(batches, data, word_idx, k_size, memory_size, output_size, vectorizer=vectorize_data_clicr_kvpointer, shuffle=False):
    # batches are of form : [(0,2), (2,4),...]
    if shuffle:
        np.random.shuffle(batches)
    for s_batch, e_batch in batches:
        dataK, dataV, dataQ, dataA, dataPM, dataKM, dataQM, dataAA = vectorizer(data[s_batch:e_batch], word_idx, output_size, k_size, memory_size)
        dataA, dataQ, dataK, dataV, dataPM, dataKM, dataQM, dataAA = extract_tensors_kvpointer(dataA, dataQ, dataK, dataV, dataPM, dataKM, dataQM, dataAA)
...
def extract_tensors_kvpointer(A, Q, K, V, PM, KM, QM, AA):
    A = torch.from_numpy(A).type(long_tensor_type)
    K = torch.from_numpy(K).type(long_tensor_type)
    V = torch.from_numpy(V).type(long_tensor_type)
    Q = np.expand_dims(Q, 1)
